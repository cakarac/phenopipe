...
class BigQueryConnection(QueryConnection):
    ...
    #bucket name if mounted to the workspace
    ...
    def get_most_recent_cache(self):
        try:
            self.log_dat = (pl.read_csv(f'{self.bucket_name}/{self.cache_loc}/log_dat.csv').with_columns(pl.col("query_id").cast(pl.Int32)))
    ...
    def remove_cached_query(self, query):
        ...
        if not cache_exists:
            ...
            self.log_dat.filter(pl.col("query_id") != cache_id).write_csv(f'{self.bucket_name}/{self.cache_loc}/log_dat.csv')
    ...
    def get_cache(self, query, lazy):
        ...
        if cache_exists.shape[0] > 0:
            ...
            dat = pl.scan_csv(f'{self.bucket_name}/{self.cache_loc}/{cache["query_path"]}')
            if not lazy:
                try:
                    dat = dat.collect()
                except ComputeError:
                    print("An issue occured while inferring the schema of cached files so schema inference is omitted!")
                    dat = pl.scan_csv(f'{self.bucket_name}/{self.cache_loc}/{cache["query_path"]}', infer_schema = False).collect()
            return dat
        else:
            return None 
    def save_cache(self, dat, query, cache_id):
        self.get_most_recent_cache()
        dat.write_csv(file = f'{self.bucket_name}/{self.cache_loc}/{cache_id}.csv')
        (self.log_dat.vstack(
            pl.DataFrame(
                ...
                .write_csv(f'{self.bucket_name}/{self.cache_loc}/log_dat.csv')
        )
